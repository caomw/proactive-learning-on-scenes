...
public:
	...
	static void saveToFile(const std::string& filename, const PointCloud<FloatType>& pointCloud) {
      ...
      writeToPLYAscii(filename, pointCloud);
		} else {
			throw MLIB_EXCEPTION("unknown file extension" + filename);
		}
	}
	...
	/* Write Functions													    */
	/************************************************************************/
  static void writeToPLYAscii( const std::string& filename,  const PointCloud<FloatType>& mesh)
  {
    ...
    file << "element vertex " << mesh.m_points.size() << "\n";
    ...
    for (size_t i = 0; i < mesh.m_points.size(); ++i)
    {
      const point3d<FloatType> &m_p = mesh.m_points[i];
      const point3d<FloatType> &m_n = mesh.m_normals[i];
      const point4d<FloatType> &m_c = mesh.m_colors[i];

      file<<m_p.x <<" " <<m_p.y <<" " <<m_p.z <<" "
        <<m_n.x <<" " <<m_n.y <<" " <<m_n.z <<" "
        <<m_c.x <<" " <<m_c.y <<" " <<m_c.z <<" ";
    }
